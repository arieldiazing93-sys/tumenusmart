 ...
 * agregados" de cada producto, así que corregir un ítem acá lo corrige en
 * todos los productos que lo usan, sin recargarlo a mano en cada uno. A
 * diferencia de los agregados propios de un producto (ProductOption,
 * gestionados en /admin/productos/[id]), esto es un catálogo aparte.
 */
export default async function GruposAgregadosPage() {
  ...
  const grupos = await prisma.optionGroup.findMany({
    ...
    include: { _count: { select: { items: true, productos: true } } },
  });
  ...
  return (
    <div>
      <Cabecera
        ...
        bajada="Un grupo con nombre (ej. Salsas, Quesos) que se adjunta a varios productos a la vez — corregir un ítem acá lo corrige en todos los productos que lo usan."
      />
