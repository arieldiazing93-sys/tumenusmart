...
export function FormularioCita({
  ...
  const estadosOfrecidos = esNueva ? ESTADOS_CITA.filter((e) => e.valor === "pendiente" || e.valor === "proxima") : ESTADOS_CITA;
  ...
  return (
    ...
    >
      <div className="flex flex-1 flex-col gap-5 overflow-y-auto px-5 py-5">
        ...
        <fieldset disabled={cobrada} className="flex min-w-0 flex-col gap-5">
          {/* ---------- id, fuente y estado ---------- */}
            ...
            <div className="rounded-lg border border-linea bg-superficie">
              <div className="flex items-center justify-between gap-2 px-3 pt-3">
                ...
                <button
                  ...
                  className="text-[0.8rem] font-semibold text-brand-texto hover:underline"
                >
              ...
              {conDescuento ? (
                <div className="px-3 pb-3 pt-2">
                  <div className="flex items-center gap-2">
                    <div role="group" aria-label="Tipo de descuento" className="flex flex-none rounded-lg border border-linea p-0.5">
                      {(["porcentaje", "monto"] as const).map((t) => (
                        <button
                          ...
                          className={`rounded-md px-3 py-1.5 text-[0.82rem] font-semibold transition-colors ${
                            tipoDescuento === t ? "bg-brand text-white" : "text-tinta-media hover:text-tinta"
                          }`}
                        >
                          {t === "porcentaje" ? "%" : "Gs."}
                        </button>
        ...
        {!esNueva && !cobrada && (
          <div className="border-t border-linea pt-5">
            <SeccionCobro valor={datosCobro} onCambio={cambiarCobro} caja={caja} total={total} cobrando={cobrando} />
          </div>
        )}
      </div>
