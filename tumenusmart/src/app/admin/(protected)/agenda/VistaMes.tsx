 ...
 *
 * En pantalla grande cada casilla lista la hora y el nombre de los primeros
 * turnos; en el celular no hay lugar para texto, así que quedan puntitos con el
 * color del estado de cada turno.
 */
export function VistaMes({
  ...
  return (
    <div>
      <div className="grid grid-cols-7 border-b border-linea bg-papel-suave text-center text-[0.68rem] font-semibold uppercase tracking-wide text-tinta-suave">
            ...
            <Link
              ...
              className={`flex min-h-[4.75rem] min-w-0 flex-col gap-1 p-1 transition-colors sm:min-h-[7rem] sm:p-1.5 ${
                esHoy
                  ...
                  : c.enMes
                    ? "bg-superficie hover:bg-papel-hundido"
            ...
            >
              <span
                className={`ml-auto flex h-6 min-w-6 flex-none items-center justify-center rounded-full px-1 text-[0.78rem] font-semibold ${
                  esHoy ? "bg-brand text-white" : c.enMes ? "text-tinta" : "text-tinta-suave"
                }`}
              >
                {c.dia}
              </span>
              ...
              <ul className="hidden min-w-0 flex-col gap-0.5 sm:flex">
                ...
                {lista.length > TURNOS_POR_DIA && (
                  <li className="text-[0.68rem] font-semibold text-brand-texto">
